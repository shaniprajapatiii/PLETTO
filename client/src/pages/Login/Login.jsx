      ...
      <div className="min-h-[calc(100vh-32px)] px-4 py-10 sm:px-6">
         <div className="mx-auto max-w-md rounded-[2rem] border border-border bg-[rgba(255,255,255,0.05)] p-8 shadow-soft backdrop-blur-xl">
            <div className="mb-8 flex items-center justify-between">
               <Logo />
               <span className="rounded-full border border-gold/30 bg-[rgba(248,181,0,0.08)] px-3 py-1 text-xs uppercase tracking-[0.2em] text-gold">Login</span>
            </div>
            ...
            <form onSubmit={submit} className="mt-8 space-y-5">
               <label className="block text-sm text-muted-foreground">
                  ...
                  <input
                     ...
                     className="mt-2 w-full rounded-3xl border border-border bg-[rgba(255,255,255,0.06)] px-4 py-3 text-white outline-none transition focus:border-gold"
                  />
               </label>
               <label className="block text-sm text-muted-foreground">
                  Password
            ...
            <p className="mt-6 text-center text-sm text-muted-foreground">
               Don’t have an account? <Link to="/register" className="text-gold hover:text-white">Create one</Link>
            </p>
         </div>
      </div>
   );
